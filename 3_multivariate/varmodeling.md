---
title: "Time Series VAR Analysis"
author: "Josephine Lukito (jlukito@wisc.edu)"
date: "March 13, 2019 | Updated: May 18, 2019"
...
```r
brexit_full <- read.csv("brexit_exercise_data.csv", header = TRUE, sep = ",", stringsAsFactors = F) %>% as.data.frame()
nrow(brexit_full)
```
...
So date is treated as a `character`. Let's turn this into a date, and remove it from our dataset (we do want this date information, but we're not actually going to include our date column in our model). Then, we'll convert the whole data frame into an `xts` object.
...
To do so, we'll use the `fracdiff` function. Important to note: this function allows you to fractionally difference. However, for this example, let's assume all the series (minus Times of India) are not-stationary; I(1).
...
The first decision we will have to make is which variables are endogenous and which are exogenous. Let's assume all the variables are endogenous except events. Though events matter, it is more likely that events would preceed activity (events may be influenced by its own media attention, but we'll set aside that question for now).

We also want to make sure we remember the order of our variables. This will be important for subsequent interpretation of the results. Our model will order the variables like this: The Guardian, NYT, Times of India, German media, and public approval of the Brexit plan.
...
var.endo <- data.frame(guardian = fd_guardian, 
                       ...
                       nyt = fd_nyt, 
                       germany = fd_germany, 
                       p_approval = fd_approval) #you'll need one data frame for endogenous variables
var.exo <- data.frame(event = brexit_ts$event) #and one for exogenous ones
```
...
Let's now run the VAR model! We will rely here on the `vars` package.
...
#Granger causality
Wikipedia: [https://en.wikipedia.org/wiki/Granger_causality](https://en.wikipedia.org/wiki/Granger_causality){target="_blank"}
...
For the `grangertest()` function, your dependent variable (DV) will be on the left, and your independent variable (IV) will be on the right. This test will compare whether a model predicting your IV is better with the inclusion of lagged variables of your DV (or whether the lagged variables of your IV do enough). 

`Grangertest()` in `lmtest` package: [https://rdrr.io/cran/lmtest/man/grangertest.html](https://rdrr.io/cran/lmtest/man/grangertest.html){target="_blank"}
`causality()` in `vars` package: [https://www.rdocumentation.org/packages/vars/versions/1.5-3/topics/causality](https://www.rdocumentation.org/packages/vars/versions/1.5-3/topics/causality){target="_blank"}
...
```r
##try these tests out to check other relationships!
...
![plot of chunk irf](figure/irf-1.png)

```r
vars::irf(var_results, impulse = "guardian", response = "timesofindia", n.ahead = 10, cumulative = F, ortho = T) %>% plot()
```

![plot of chunk irf](figure/irf-2.png)

```r
#vars::irf(var_results, impulse = "guardian", response = c("timesofindia", "nyt"), n.ahead = 10, cumulative = F, ortho = T) %>% plot()
```
...
#Post-Hoc Tests
A common post-hoc test for autocorrelation is the Durbin-Watson. You do *not* want autocorrelation in your residuals.
